...
ブランチ: `claude/magic-restaurant-data-update`（`main`から分岐。CISSP学習サービス（`claude/cissp-learning-service-gr4asr`ブランチ）とは分離済み）
...
1. ✅ **データ収集リポジトリのコミット**（2026-06-18完了。ローカルコミット済み、origin未push分は別途）
2. ✅ **アプリ側のコミット**（`claude/magic-restaurant-data-update`ブランチに分離・コミット済み）
3. ✅ **ブランチ運用の整理**（`main`から新ブランチを切り、CISSPと分離済み）
4. ✅ **メタデータ修正**（title/description/lang="ja"/OGP・Twitterカード追加、2026-06-19完了）
5. **【未着手】GitHubへpush**
   - データ収集リポジトリ: `claude/magic-restaurant-scraping-nujto9` に6コミット分（5+1）の未push分あり
   - アプリ本体: `claude/magic-restaurant-data-update` ブランチ自体が未push（新規ブランチのためpushが必要）
6. **【未着手】Vercel本番デプロイ**
   プロジェクトは作成済み（`.vercel/project.json`あり）だが本デプロイ未実施。まずPreviewデプロイで確認後、本番判断する。
7. **OGP画像**
   現状テキストのみのOGP。画像（`opengraph-image`）は未設定（任意・将来対応）。
8. **ジャンル「その他」残り148件（5.3%）**
