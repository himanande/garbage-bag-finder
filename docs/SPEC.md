...
3. ✅ **AdSense設置の準備**（`/ads.txt`とスクリプト読み込みを`NEXT_PUBLIC_ADSENSE_PUBLISHER_ID`環境変数で出し分け。パブリッシャーID未設定。ユーザーがAdSenseダッシュボードでサイト追加申請→審査通過後にID設定で有効化）
4. ✅ **アフィリエイトプログラム調査**（バリューコマース経由で食べログ（即時提携）・ホットペッパーグルメ（成果¥100/件）等が利用可能と判明。LinkSwitch機能で個別店舗の提携ID管理不要。ユーザーがバリューコマース登録・提携申請待ち、実装は申請後）
