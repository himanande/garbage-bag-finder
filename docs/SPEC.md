...
### 9-2. 進行中・未着手（優先度順）
1. **独自ドメイン取得**（`mahoresu-search.com`に決定、$11.25/年。Vercelの安全機構によりエージェントは購入不可、ユーザーが`npx vercel domains buy mahoresu-search.com`を対話実行 or ダッシュボードで購入。購入後はVercelでドメイン追加 + `NEXT_PUBLIC_SITE_URL`環境変数を更新）
...
3. **AdSense設置**（既存アカウントに本サイトを追加申請 → 審査通過後 `ads.txt` 設置・広告コード組み込み）
4. **アフィリエイトプログラム調査**（ぐるなび・食べログ等の提携プログラムを調査し、利用可否を検討）
5. **ジャンル「その他」残り148件（5.3%）**（同名店舗の一意特定不可・既存16ジャンルに当てはまらない業態が中心。これ以上の自動改善は限界）
6. **OGP画像**（現状テキストのみ。`opengraph-image`は任意・将来対応）
7. **お問い合わせ手段の整備**（`/about`に「削除依頼は今後追記予定」と記載したまま。連絡先確定後に追記）
